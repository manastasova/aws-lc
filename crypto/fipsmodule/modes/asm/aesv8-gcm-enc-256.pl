...
sub inc_ctr {
    return "add\t$rtmp_ctr.4s, $rtmp_ctr.4s, $rctr_inc.4s";
}

sub rev_ctr {
    my ($dst) = @_;
    return "rev32\t$dst, $rtmp_ctr.16b";
}

sub mod_ldr {
    return "ldr\t$mod_constantd, [$modulo_constant]";
}

sub mod_ext_acc_h {
    return "ext\t$t11.16b, $acc_hb, $acc_hb, #8";
}

sub mod_pmull_top {
    return "pmull\t$t12.1q, $acc_h.1d, $mod_constant.1d";
}

sub mod_karatsuba {
    return "eor3\t$acc_mb, $acc_mb, $acc_hb, $acc_lb";
}

sub mod_fold_mid {
    return "eor3\t$acc_mb, $acc_mb, $t12.16b, $t11.16b";
}

sub mod_ext_acc_m {
    return "ext\t$t11.16b, $acc_mb, $acc_mb, #8";
}

sub mod_pmull_mid {
    return "pmull\t$acc_h.1q, $acc_m.1d, $mod_constant.1d";
}

sub mod_fold_low {
    return "eor3\t$acc_lb, $acc_lb, $t11.16b, $acc_hb";
}
#########################################################################################
...
aesv8_gcm_8x_enc_256:
	...
	`&inc_ctr()`		@ CTR block 0
	`&rev_ctr("$ctr1.16b")`				@ CTR block 1

	`&inc_ctr()`		@ CTR block 1
	`&rev_ctr("$ctr2.16b")`				@ CTR block 2

	`&inc_ctr()`		@ CTR block 2
	`&rev_ctr("$ctr3.16b")`				@ CTR block 3

	`&inc_ctr()`		@ CTR block 3
	`&rev_ctr("$ctr4.16b")`				@ CTR block 4
	`&inc_ctr()`		@ CTR block 4

	`&rev_ctr("$ctr5.16b")`				@ CTR block 5
	`&inc_ctr()`		@ CTR block 5
	
	ldp	$rk0q, $rk1q, [$cc, #0]				 	@ load rk0, rk1

	`&rev_ctr("$ctr6.16b")`				@ CTR block 6
	`&inc_ctr()`		@ CTR block 6

	`&rev_ctr("$ctr7.16b")`				@ CTR block 7

	`&aes_round("$ctr2b", "$rk0")`			@ AES block 2 - round 0
	`&aes_round("$ctr3b", "$rk0")`			@ AES block 3 - round 0
	`&aes_round("$ctr4b", "$rk0")`			@ AES block 4 - round 0

	`&aes_round("$ctr0b", "$rk0")`			@ AES block 0 - round 0
	`&aes_round("$ctr1b", "$rk0")`			@ AES block 1 - round 0
	`&aes_round("$ctr6b", "$rk0")`			@ AES block 6 - round 0

	`&aes_round("$ctr5b", "$rk0")`			@ AES block 5 - round 0
	`&aes_round("$ctr7b", "$rk0")`			@ AES block 7 - round 0
	ldp	$rk2q, $rk3q, [$cc, #32]				@ load rk2, rk3

	`&aes_round("$ctr1b", "$rk1")`			@ AES block 1 - round 1
	`&aes_round("$ctr3b", "$rk1")`			@ AES block 3 - round 1
	`&aes_round("$ctr4b", "$rk1")`			@ AES block 4 - round 1

	`&aes_round("$ctr5b", "$rk1")`			@ AES block 5 - round 1
	`&aes_round("$ctr6b", "$rk1")`			@ AES block 6 - round 1

	`&aes_round("$ctr2b", "$rk1")`			@ AES block 2 - round 1

	`&aes_round("$ctr7b", "$rk1")`			@ AES block 7 - round 1

	`&aes_round("$ctr2b", "$rk2")`			@ AES block 2 - round 2
	`&aes_round("$ctr3b", "$rk2")`			@ AES block 3 - round 2
	`&aes_round("$ctr0b", "$rk1")`			@ AES block 0 - round 1

	`&aes_round("$ctr5b", "$rk2")`			@ AES block 5 - round 2
	`&aes_round("$ctr6b", "$rk2")`			@ AES block 6 - round 2
	`&aes_round("$ctr7b", "$rk2")`			@ AES block 7 - round 2

	`&aes_round("$ctr0b", "$rk2")`			@ AES block 0 - round 2
	`&aes_round("$ctr1b", "$rk2")`			@ AES block 1 - round 2
	`&aes_round("$ctr4b", "$rk2")`			@ AES block 4 - round 2

	`&aes_round("$ctr3b", "$rk3")`			@ AES block 3 - round 3
	`&aes_round("$ctr5b", "$rk3")`			@ AES block 5 - round 3
	ldp	$rk4q, $rk5q, [$cc, #64]				@ load rk4, rk5

	`&aes_round("$ctr4b", "$rk3")`			@ AES block 4 - round 3

	`&aes_round("$ctr1b", "$rk3")`			@ AES block 1 - round 3
	`&aes_round("$ctr6b", "$rk3")`			@ AES block 6 - round 3
	`&aes_round("$ctr7b", "$rk3")`			@ AES block 7 - round 3

	`&aes_round("$ctr0b", "$rk3")`			@ AES block 0 - round 3
	`&aes_round("$ctr2b", "$rk3")`			@ AES block 2 - round 3

	`&aes_round("$ctr1b", "$rk4")`			@ AES block 1 - round 4
	`&aes_round("$ctr4b", "$rk4")`			@ AES block 4 - round 4
	`&aes_round("$ctr6b", "$rk4")`			@ AES block 6 - round 4

	`&aes_round("$ctr0b", "$rk4")`			@ AES block 0 - round 4
	`&aes_round("$ctr2b", "$rk4")`			@ AES block 2 - round 4

	`&aes_round("$ctr3b", "$rk4")`			@ AES block 3 - round 4
	`&aes_round("$ctr5b", "$rk4")`			@ AES block 5 - round 4
	`&aes_round("$ctr7b", "$rk4")`			@ AES block 7 - round 4

	`&aes_round("$ctr0b", "$rk5")`			@ AES block 0 - round 5
	`&aes_round("$ctr2b", "$rk5")`			@ AES block 2 - round 5
	ldp	$rk6q, $rk7q, [$cc, #96]				@ load rk6, rk7

	`&aes_round("$ctr1b", "$rk5")`			@ AES block 1 - round 5
	`&aes_round("$ctr4b", "$rk5")`			@ AES block 4 - round 5
	`&aes_round("$ctr5b", "$rk5")`			@ AES block 5 - round 5

	`&aes_round("$ctr3b", "$rk5")`			@ AES block 3 - round 5
	`&aes_round("$ctr6b", "$rk5")`			@ AES block 6 - round 5
	`&aes_round("$ctr7b", "$rk5")`			@ AES block 7 - round 5

	`&aes_round("$ctr1b", "$rk6")`			@ AES block 1 - round 6
	`&aes_round("$ctr4b", "$rk6")`			@ AES block 4 - round 6
	`&aes_round("$ctr5b", "$rk6")`			@ AES block 5 - round 6

	`&aes_round("$ctr0b", "$rk6")`			@ AES block 0 - round 6
	`&aes_round("$ctr2b", "$rk6")`			@ AES block 2 - round 6
	`&aes_round("$ctr6b", "$rk6")`			@ AES block 6 - round 6

	`&aes_round("$ctr3b", "$rk6")`			@ AES block 3 - round 6
	`&aes_round("$ctr7b", "$rk6")`			@ AES block 7 - round 6
	ldp	$rk8q, $rk9q, [$cc, #128]				@ load rk8, rk9

	`&aes_round("$ctr0b", "$rk7")`			@ AES block 0 - round 7
	`&aes_round("$ctr2b", "$rk7")`			@ AES block 2 - round 7

	`&aes_round("$ctr1b", "$rk7")`			@ AES block 1 - round 7
	`&aes_round("$ctr6b", "$rk7")`			@ AES block 6 - round 7
	`&aes_round("$ctr7b", "$rk7")`			@ AES block 7 - round 7

	`&aes_round("$ctr3b", "$rk7")`			@ AES block 3 - round 7
	`&aes_round("$ctr5b", "$rk7")`			@ AES block 5 - round 7

	`&aes_round("$ctr4b", "$rk7")`			@ AES block 4 - round 7

	`&aes_round("$ctr1b", "$rk8")`			@ AES block 1 - round 8
	`&aes_round("$ctr6b", "$rk8")`			@ AES block 6 - round 8

	`&aes_round("$ctr0b", "$rk8")`			@ AES block 0 - round 8
	`&aes_round("$ctr3b", "$rk8")`			@ AES block 3 - round 8
	`&aes_round("$ctr7b", "$rk8")`			@ AES block 7 - round 8

	`&aes_round("$ctr2b", "$rk8")`			@ AES block 2 - round 8
	`&aes_round("$ctr4b", "$rk8")`			@ AES block 4 - round 8
	`&aes_round("$ctr5b", "$rk8")`			@ AES block 5 - round 8
	...
	`&aes_round("$ctr3b", "$rk9")`			@ AES block 3 - round 9
	`&aes_round("$ctr6b", "$rk9")`			@ AES block 6 - round 9
	`&aes_round("$ctr7b", "$rk9")`			@ AES block 7 - round 9

	`&aes_round("$ctr2b", "$rk9")`			@ AES block 2 - round 9
	`&aes_round("$ctr4b", "$rk9")`			@ AES block 4 - round 9
	`&aes_round("$ctr5b", "$rk9")`			@ AES block 5 - round 9

	`&aes_round("$ctr1b", "$rk9")`			@ AES block 1 - round 9

	`&aes_round("$ctr4b", "$rk10")`			@ AES block 4 - round 10
	`&aes_round("$ctr7b", "$rk10")`			@ AES block 7 - round 10
	`&aes_round("$ctr0b", "$rk9")`			@ AES block 0 - round 9

	`&aes_round("$ctr1b", "$rk10")`			@ AES block 1 - round 10
	`&aes_round("$ctr3b", "$rk10")`			@ AES block 3 - round 10
	`&aes_round("$ctr5b", "$rk10")`			@ AES block 5 - round 10

	`&aes_round("$ctr0b", "$rk10")`			@ AES block 0 - round 10
	`&aes_round("$ctr2b", "$rk10")`			@ AES block 2 - round 10
	`&aes_round("$ctr6b", "$rk10")`			@ AES block 6 - round 10

	`&aes_round("$ctr4b", "$rk11")`			@ AES block 4 - round 11
	ldp	$rk12q, $rk13q, [$cc, #192]				@ load rk12, rk13
	`&aes_round("$ctr5b", "$rk11")`			@ AES block 5 - round 11

	`&aes_round("$ctr1b", "$rk11")`			@ AES block 1 - round 11
	`&aes_round("$ctr2b", "$rk11")`			@ AES block 2 - round 11
	`&aes_round("$ctr6b", "$rk11")`			@ AES block 6 - round 11

	`&aes_round("$ctr0b", "$rk11")`			@ AES block 0 - round 11
	`&aes_round("$ctr3b", "$rk11")`			@ AES block 3 - round 11
	`&aes_round("$ctr7b", "$rk11")`			@ AES block 7 - round 11

	`&inc_ctr()`		@ CTR block 7
	ldr	$rk14q, [$cc, #224]					@ load rk14

	`&aes_round("$ctr1b", "$rk12")`			@ AES block 1 - round 12
	`&aes_round("$ctr2b", "$rk12")`			@ AES block 2 - round 12
	`&aes_round("$ctr4b", "$rk12")`			@ AES block 4 - round 12

	`&aes_round("$ctr0b", "$rk12")`			@ AES block 0 - round 12
	`&aes_round("$ctr3b", "$rk12")`			@ AES block 3 - round 12
	`&aes_round("$ctr5b", "$rk12")`			@ AES block 5 - round 12

	`&aes_round_last("$ctr1b", "$rk13")`						@ AES block 1 - round 13
	`&aes_round_last("$ctr2b", "$rk13")`						@ AES block 2 - round 13
	`&aes_round_last("$ctr4b", "$rk13")`						@ AES block 4 - round 13

	`&aes_round("$ctr6b", "$rk12")`			@ AES block 6 - round 12
	`&aes_round("$ctr7b", "$rk12")`			@ AES block 7 - round 12

	`&aes_round_last("$ctr0b", "$rk13")`						@ AES block 0 - round 13
	`&aes_round_last("$ctr5b", "$rk13")`						@ AES block 5 - round 13

	`&aes_round_last("$ctr3b", "$rk13")`						@ AES block 3 - round 13
	`&aes_round_last("$ctr6b", "$rk13")`						@ AES block 6 - round 13
	`&aes_round_last("$ctr7b", "$rk13")`						@ AES block 7 - round 13
...
.L256_enc_main_loop:							@ main loop start
	...
	`&rev_ctr("$ctr6.16b")`				@ CTR block 8k+14
	`&inc_ctr()`		@ CTR block 8k+14
	...
	`&aes_round("$ctr3b", "$rk0")`			@ AES block 8k+11 - round 0
	`&aes_round("$ctr5b", "$rk0")`			@ AES block 8k+13 - round 0
	`&rev_ctr("$ctr7.16b")`				@ CTR block 8k+15

	`&aes_round("$ctr0b", "$rk0")`			@ AES block 8k+8 - round 0
	`&aes_round("$ctr1b", "$rk0")`			@ AES block 8k+9 - round 0
	`&aes_round("$ctr6b", "$rk0")`			@ AES block 8k+14 - round 0

	`&aes_round("$ctr2b", "$rk0")`			@ AES block 8k+10 - round 0
	`&aes_round("$ctr4b", "$rk0")`			@ AES block 8k+12 - round 0
	`&aes_round("$ctr7b", "$rk0")`			@ AES block 8k+15 - round 0

	ldp	$rk2q, $rk3q, [$cc, #32]				@ load rk2, rk3
	eor	$res0b, $res0b, $acc_lb				 	@ PRE 1
	`&aes_round("$ctr6b", "$rk1")`			@ AES block 8k+14 - round 1

	`&aes_round("$ctr0b", "$rk1")`			@ AES block 8k+8 - round 1
	`&aes_round("$ctr1b", "$rk1")`			@ AES block 8k+9 - round 1
	`&aes_round("$ctr2b", "$rk1")`			@ AES block 8k+10 - round 1

	`&aes_round("$ctr3b", "$rk1")`			@ AES block 8k+11 - round 1
	`&aes_round("$ctr4b", "$rk1")`			@ AES block 8k+12 - round 1
	`&aes_round("$ctr5b", "$rk1")`			@ AES block 8k+13 - round 1
	...
	`&aes_round("$ctr7b", "$rk1")`			@ AES block 8k+15 - round 1

	`&aes_round("$ctr1b", "$rk2")`			@ AES block 8k+9 - round 2
	`&aes_round("$ctr5b", "$rk2")`			@ AES block 8k+13 - round 2
	`&aes_round("$ctr6b", "$rk2")`			@ AES block 8k+14 - round 2

	`&aes_round("$ctr2b", "$rk2")`			@ AES block 8k+10 - round 2
	pmull	$h7.1q, $res1.1d, $h7.1d				@ GHASH block 8k+1 - low
	`&aes_round("$ctr4b", "$rk2")`			@ AES block 8k+12 - round 2

	`&aes_round("$ctr5b", "$rk3")`			@ AES block 8k+13 - round 3
	`&aes_round("$ctr6b", "$rk3")`			@ AES block 8k+14 - round 3
	`&aes_round("$ctr0b", "$rk2")`			@ AES block 8k+8 - round 2

	`&aes_round("$ctr1b", "$rk3")`			@ AES block 8k+9 - round 3
	`&aes_round("$ctr3b", "$rk2")`			@ AES block 8k+11 - round 2
	`&aes_round("$ctr7b", "$rk2")`			@ AES block 8k+15 - round 2

	`&aes_round("$ctr4b", "$rk3")`			@ AES block 8k+12 - round 3
	rev64	$res6b, $res6b						@ GHASH block 8k+6
	pmull2  $t2.1q, $res3.2d, $h5.2d				@ GHASH block 8k+3 - high

	`&aes_round("$ctr3b", "$rk3")`			@ AES block 8k+11 - round 3
	ldp	$rk4q, $rk5q, [$cc, #64]				@ load rk4, rk5
	rev64	$res2b, $res2b						@ GHASH block 8k+2

	`&aes_round("$ctr0b", "$rk3")`			@ AES block 8k+8 - round 3
	`&aes_round("$ctr2b", "$rk3")`			@ AES block 8k+10 - round 3
	`&aes_round("$ctr7b", "$rk3")`			@ AES block 8k+15 - round 3
	...
	`&aes_round("$ctr1b", "$rk4")`			@ AES block 8k+9 - round 4
	`&aes_round("$ctr4b", "$rk4")`			@ AES block 8k+12 - round 4
	`&aes_round("$ctr5b", "$rk4")`			@ AES block 8k+13 - round 4

	`&aes_round("$ctr2b", "$rk4")`			@ AES block 8k+10 - round 4
	`&aes_round("$ctr3b", "$rk4")`			@ AES block 8k+11 - round 4
	`&aes_round("$ctr7b", "$rk4")`			@ AES block 8k+15 - round 4

	trn1	$t3.2d, $res3.2d, $res2.2d				@ GHASH block 8k+2, 8k+3 - mid
	`&aes_round("$ctr0b", "$rk4")`			@ AES block 8k+8 - round 4
	`&aes_round("$ctr6b", "$rk4")`			@ AES block 8k+14 - round 4

	trn2	$res2.2d, $res3.2d, $res2.2d				@ GHASH block 8k+2, 8k+3 - mid
	eor	$res0.16b, $res0.16b, $acc_m.16b			@ GHASH block 8k, 8k+1 - mid
	ldp	$rk6q, $rk7q, [$cc, #96]				@ load rk6, rk7

	`&aes_round("$ctr4b", "$rk5")`			@ AES block 8k+12 - round 5
	`&aes_round("$ctr5b", "$rk5")`			@ AES block 8k+13 - round 5
	`&aes_round("$ctr7b", "$rk5")`			@ AES block 8k+15 - round 5

	eor	$res2.16b, $res2.16b, $t3.16b				@ GHASH block 8k+2, 8k+3 - mid
	`&aes_round("$ctr2b", "$rk5")`			@ AES block 8k+10 - round 5
	rev64	$res7b, $res7b						@ GHASH block 8k+7

	`&aes_round("$ctr1b", "$rk5")`			@ AES block 8k+9 - round 5
	`&aes_round("$ctr3b", "$rk5")`			@ AES block 8k+11 - round 5
	`&aes_round("$ctr6b", "$rk5")`			@ AES block 8k+14 - round 5

	pmull2  $t3.1q, $res2.2d, $h56k.2d				@ GHASH block 8k+2 - mid
	pmull2  $acc_m.1q, $res0.2d, $h78k.2d				@ GHASH block 8k	- mid
	`&aes_round("$ctr0b", "$rk5")`			@ AES block 8k+8 - round 5

	pmull	$h78k.1q, $res0.1d, $h78k.1d				@ GHASH block 8k+1 - mid
	`&aes_round("$ctr2b", "$rk6")`			@ AES block 8k+10 - round 6
	`&aes_round("$ctr4b", "$rk6")`			@ AES block 8k+12 - round 6

	`&aes_round("$ctr1b", "$rk6")`			@ AES block 8k+9 - round 6
	`&aes_round("$ctr6b", "$rk6")`			@ AES block 8k+14 - round 6
	`&aes_round("$ctr7b", "$rk6")`			@ AES block 8k+15 - round 6

	eor	$acc_mb, $acc_mb, $h78k.16b				@ GHASH block 8k+1 - mid
	pmull	$h56k.1q, $res2.1d, $h56k.1d				@ GHASH block 8k+3 - mid
	`&aes_round("$ctr5b", "$rk6")`			@ AES block 8k+13 - round 6

	eor3	$acc_lb, $acc_lb, $h6.16b, $h5.16b			@ GHASH block 8k+2, 8k+3 - low
	`&aes_round("$ctr0b", "$rk6")`			@ AES block 8k+8 - round 6
	`&aes_round("$ctr3b", "$rk6")`			@ AES block 8k+11 - round 6

	ldp	$rk8q, $rk9q, [$cc, #128]				@ load rk8, rk9
	pmull2  $t4.1q, $res4.2d, $h4.2d				@ GHASH block 8k+4 - high
	`&aes_round("$ctr5b", "$rk7")`			@ AES block 8k+13 - round 7

	ldr	$h1q, [$Htable]				@ load h1l | h1h
	ldr	$h2q, [$Htable, #32]				@ load h2l | h2h
	`&aes_round("$ctr2b", "$rk7")`			@ AES block 8k+10 - round 7
	...
	`&aes_round("$ctr3b", "$rk7")`			@ AES block 8k+11 - round 7
	`&aes_round("$ctr6b", "$rk7")`			@ AES block 8k+14 - round 7

	`&aes_round("$ctr0b", "$rk7")`			@ AES block 8k+8 - round 7
	`&aes_round("$ctr7b", "$rk7")`			@ AES block 8k+15 - round 7
	pmull	$h4.1q, $res4.1d, $h4.1d				@ GHASH block 8k+4 - low

	trn2	$res4.2d, $res5.2d, $res4.2d				@ GHASH block 8k+4, 8k+5 - mid
	`&aes_round("$ctr1b", "$rk7")`			@ AES block 8k+9 - round 7
	`&aes_round("$ctr4b", "$rk7")`			@ AES block 8k+12 - round 7

	pmull2  $t5.1q, $res5.2d, $h3.2d				@ GHASH block 8k+5 - high
	`&aes_round("$ctr0b", "$rk8")`			@ AES block 8k+8 - round 8
	`&aes_round("$ctr7b", "$rk8")`			@ AES block 8k+15 - round 8
	...
	`&aes_round("$ctr3b", "$rk8")`			@ AES block 8k+11 - round 8
	`&aes_round("$ctr0b", "$rk9")`			@ AES block 8k+8 - round 9
	`&aes_round("$ctr1b", "$rk8")`			@ AES block 8k+9 - round 8

	pmull2  $t6.1q, $res4.2d, $h34k.2d				@ GHASH block 8k+4 - mid
	pmull	$h34k.1q, $res4.1d, $h34k.1d				@ GHASH block 8k+5 - mid
	`&aes_round("$ctr2b", "$rk8")`			@ AES block 8k+10 - round 8

	`&aes_round("$ctr5b", "$rk8")`			@ AES block 8k+13 - round 8
	pmull2  $t7.1q, $res6.2d, $h2.2d				@ GHASH block 8k+6 - high
	pmull	$h2.1q, $res6.1d, $h2.1d				@ GHASH block 8k+6 - low

	`&aes_round("$ctr6b", "$rk8")`			@ AES block 8k+14 - round 8
	trn2	$res6.2d, $res7.2d, $res6.2d				@ GHASH block 8k+6, 8k+7 - mid
	`&aes_round("$ctr4b", "$rk8")`			@ AES block 8k+12 - round 8

	eor3	$acc_mb, $acc_mb, $h34k.16b, $t6.16b			@ GHASH block 8k+4, 8k+5 - mid
	`&aes_round("$ctr5b", "$rk9")`			@ AES block 8k+13 - round 9
	`&aes_round("$ctr7b", "$rk9")`			@ AES block 8k+15 - round 9

	eor	$res6.16b, $res6.16b, $t9.16b				@ GHASH block 8k+6, 8k+7 - mid
	`&aes_round("$ctr4b", "$rk9")`			@ AES block 8k+12 - round 9
	`&aes_round("$ctr6b", "$rk9")`			@ AES block 8k+14 - round 9

	ldp	$rk10q, $rk11q, [$cc, #160]				@ load rk10, rk11
	`&aes_round("$ctr2b", "$rk9")`			@ AES block 8k+10 - round 9
	`&aes_round("$ctr3b", "$rk9")`			@ AES block 8k+11 - round 9
	...
	`&aes_round("$ctr3b", "$rk10")`			@ AES block 8k+11 - round 10
	`&aes_round("$ctr4b", "$rk10")`			@ AES block 8k+12 - round 10
	`&aes_round("$ctr5b", "$rk10")`			@ AES block 8k+13 - round 10

	`&aes_round("$ctr0b", "$rk10")`			@ AES block 8k+8 - round 10
	`&aes_round("$ctr2b", "$rk10")`			@ AES block 8k+10 - round 10
	`&inc_ctr()`		@ CTR block 8k+15

	`&aes_round("$ctr1b", "$rk10")`			@ AES block 8k+9 - round 10
	`&aes_round("$ctr6b", "$rk10")`			@ AES block 8k+14 - round 10
	`&aes_round("$ctr7b", "$rk10")`			@ AES block 8k+15 - round 10

	eor3	$acc_hb, $acc_hb, $t7.16b, $t8.16b			@ GHASH block 8k+6, 8k+7 - high

	ldp	$rk12q, $rk13q, [$cc, #192]				@ load rk12, rk13
	`&rev_ctr("$h1.16b")`					@ CTR block 8k+16

	`&mod_ext_acc_h()`			 	@ MODULO - other top alignment
	ldp	$ctr_t0q, $ctr_t1q, [$input_ptr], #32			@ AES block 8k+8, 8k+9 - load plaintext
	`&aes_round("$ctr2b", "$rk11")`			@ AES block 8k+10 - round 11

	`&aes_round("$ctr6b", "$rk11")`			@ AES block 8k+14 - round 11
	`&inc_ctr()`		@ CTR block 8k+16
	`&aes_round("$ctr3b", "$rk11")`			@ AES block 8k+11 - round 11

	`&aes_round("$ctr0b", "$rk11")`			@ AES block 8k+8 - round 11
	`&aes_round("$ctr7b", "$rk11")`			@ AES block 8k+15 - round 11

	`&mod_pmull_top()`		 	@ MODULO - top 64b align with mid
	`&aes_round("$ctr1b", "$rk11")`			@ AES block 8k+9 - round 11

	`&aes_round("$ctr7b", "$rk12")`			@ AES block 8k+15 - round 12
	`&aes_round("$ctr5b", "$rk11")`			@ AES block 8k+13 - round 11

	`&aes_round("$ctr3b", "$rk12")`			@ AES block 8k+11 - round 12
	`&aes_round("$ctr6b", "$rk12")`			@ AES block 8k+14 - round 12
	`&rev_ctr("$h2.16b")`					@ CTR block 8k+17

	`&inc_ctr()`		@ CTR block 8k+17
	`&aes_round("$ctr4b", "$rk11")`			@ AES block 8k+12 - round 11
	`&mod_karatsuba()`		 	@ MODULO - karatsuba tidy up

	`&aes_round("$ctr5b", "$rk12")`			@ AES block 8k+13 - round 12
	ldr	$rk14q, [$cc, #224]					@ load rk14
	`&aes_round_last("$ctr7b", "$rk13")`						@ AES block 8k+15 - round 13

	ldp	$ctr_t2q, $ctr_t3q, [$input_ptr], #32			@ AES block 8k+10, 8k+11 - load plaintext
	`&aes_round("$ctr2b", "$rk12")`			@ AES block 8k+10 - round 12
	`&aes_round("$ctr4b", "$rk12")`			@ AES block 8k+12 - round 12

	`&mod_fold_mid()`			@ MODULO - fold into mid
	`&aes_round("$ctr1b", "$rk12")`			@ AES block 8k+9 - round 12
	ldp	$ctr_t4q, $ctr_t5q, [$input_ptr], #32			@ AES block 4, 5 - load plaintext

	ldp	$ctr_t6q, $ctr_t7q, [$input_ptr], #32			@ AES block 6, 7 - load plaintext
	`&aes_round_last("$ctr2b", "$rk13")`						@ AES block 8k+10 - round 13
	`&aes_round_last("$ctr4b", "$rk13")`						@ AES block 8k+12 - round 13

	`&rev_ctr("$h3.16b")`					@ CTR block 8k+18
	`&inc_ctr()`		@ CTR block 8k+18
	`&aes_round_last("$ctr5b", "$rk13")`						@ AES block 8k+13 - round 13

	`&aes_round("$ctr0b", "$rk12")`			@ AES block 8k+8 - round 12
	`&aes_round_last("$ctr3b", "$rk13")`						@ AES block 8k+11 - round 13
	cmp	$input_ptr, $main_end_input_ptr				@ LOOP CONTROL

	eor3	$res2b, $ctr_t2b, $ctr2b, $rk14				@ AES block 8k+10 - result
	`&rev_ctr("$h4.16b")`					@ CTR block 8k+19
	`&inc_ctr()`		@ CTR block 8k+19

	`&aes_round_last("$ctr0b", "$rk13")`						@ AES block 8k+8 - round 13
	`&aes_round_last("$ctr6b", "$rk13")`						@ AES block 8k+14 - round 13
	eor3	$res5b, $ctr_t5b, $ctr5b, $rk14				@ AES block 5 - result

	`&mod_ext_acc_m()`				@ MODULO - other mid alignment
	`&mod_pmull_mid()`			@ MODULO - mid 64b align with low
	`&aes_round_last("$ctr1b", "$rk13")`						@ AES block 8k+9 - round 13

	eor3	$res4b, $ctr_t4b, $ctr4b, $rk14				@ AES block 4 - result
	`&rev_ctr("$ctr4.16b")`				@ CTR block 8k+20
