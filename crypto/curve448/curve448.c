...
static void point_double_internal(curve448_point_t p, const curve448_point_t q,
    ...
    if (GF_HEADROOM == 5)
        gf_weak_reduce(a);
    c448_gf_mul(p->x, a, b);
    c448_gf_mul(p->z, p->t, a);
    c448_gf_mul(p->y, p->t, d);
    if (!before_double)
        c448_gf_mul(p->t, b, d);
}
...
static void pniels_to_pt(curve448_point_t e, const pniels_t d) {
    ...
    c448_gf_mul(e->t, e->y, eu);
    c448_gf_mul(e->x, d->z, e->y);
    c448_gf_mul(e->y, d->z, eu);
    c448_gf_sqr(e->z, d->z);
}
...
static void add_niels_to_pt(curve448_point_t d, const niels_t e,
    ...
    c448_gf_mul(a, e->a, b);
    gf_add_nr(b, d->x, d->y);
    c448_gf_mul(d->y, e->b, b);
    c448_gf_mul(d->x, e->c, d->t);
    gf_add_nr(c, a, d->y);
    gf_sub_nr(b, d->y, a);
    gf_sub_nr(d->y, d->z, d->x);
    gf_add_nr(a, d->x, d->z);
    c448_gf_mul(d->z, a, d->y);
    c448_gf_mul(d->x, d->y, b);
    c448_gf_mul(d->y, a, c);
    if (!before_double)
        c448_gf_mul(d->t, b, c);
}
...
c448_bool_t curve448_point_eq(const curve448_point_t p,
    ...
    c448_gf_mul(a, p->y, q->x);
    c448_gf_mul(b, q->y, p->x);
    succ = gf_eq(a, b);
...
c448_bool_t curve448_point_valid(const curve448_point_t p) {
    ...
    c448_gf_mul(a, p->x, p->y);
    c448_gf_mul(b, p->z, p->t);
    out = gf_eq(a, b);
    c448_gf_sqr(a, p->x);
    c448_gf_sqr(b, p->y);
    gf_sub(a, b, a);
...
static inline void constant_time_lookup_niels(niels_s *RESTRICT ni,
    ...
    size_t n_words = sizeof(niels_s) / sizeof(word_t);
    word_t *out = (word_t *)ni;
    int i;
    size_t j;

    memset(ni, 0, sizeof(niels_s));
    for (i = 0; i < nelts; i++) {
        mask_t mask = word_is_zero((word_t)(i ^ idx));
        const word_t *elem = (const word_t *)&table[i];
        for (j = 0; j < n_words; j++)
            out[j] |= mask & elem[j];
    }
}
...
c448_error_t curve448_point_decode_like_eddsa_and_mul_by_ratio(
    ...
    c448_gf_mul(p->x, p->z, p->t);
    succ &= gf_isr(p->t, p->x);

    c448_gf_mul(p->x, p->t, p->z);
...
c448_error_t c448_x448_int(uint8_t out[X448_PUBLIC_BYTES],
    ...
    for (t = X_PRIVATE_BITS - 1; t >= 0; t--) {
        ...
        gf_sub_nr(t2, x2, z2);
        gf_sub_nr(z2, x3, z3);
        c448_gf_mul(x2, t1, z2);
        gf_add_nr(z2, z3, x3);
        c448_gf_mul(x3, t2, z2);
        gf_sub_nr(z3, x2, x3);
        c448_gf_sqr(z2, z3);
        c448_gf_mul(z3, x1, z2);
        gf_add_nr(z2, x2, x3);
        c448_gf_sqr(x3, z2);

        c448_gf_sqr(z2, t1);
        c448_gf_sqr(t1, t2);
        c448_gf_mul(x2, z2, t1);
        gf_sub_nr(t2, z2, t1);

        gf_mulw(t1, t2, -EDWARDS_D);
        gf_add_nr(t1, t1, z2);
        c448_gf_mul(z2, t2, t1);
    }

    gf_cond_swap(x2, x3, swap);
    gf_cond_swap(z2, z3, swap);
    gf_invert(z2, z2, 0);
    c448_gf_mul(x1, x2, z2);
...
void c448_x448_derive_public_key(uint8_t out[X448_PUBLIC_BYTES],
    const uint8_t scalar[X448_PRIVATE_BYTES]) {
    uint8_t scalar2[X448_PRIVATE_BYTES];
    ...
    scalar2[X448_PRIVATE_BYTES - 1] &= ~((0u - 1u) << ((X_PRIVATE_BITS + 7) % 8));
    scalar2[X448_PRIVATE_BYTES - 1] |= 1 << ((X_PRIVATE_BITS + 7) % 8);
...
void curve448_base_double_scalarmul_non_secret(curve448_point_t combo,
    ...
    if (i > control_pre[0].power) {
        ...
        add_niels_to_pt(combo,
            curve448_wnaf_base[control_pre[0].addend >> 1], i);
        contv++;
        contp++;
    } else {
    ...
    for (i--; i >= 0; i--) {
        ...
        if (cp) {
            ...
            if (control_pre[contp].addend > 0)
                add_niels_to_pt(combo,
                    curve448_wnaf_base[control_pre[contp].addend >> 1],
                    i);
            else
